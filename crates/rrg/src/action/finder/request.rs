...
fn into_action(proto: rrg_proto::flows::FileFinderAction) -> Result<Option<Action>, crate::ParseArgsError> {
...
impl TryFrom<rrg_proto::flows::FileFinderHashActionOptions> for Action {
    type Error = crate::ParseArgsError;
...
) -> Result<Option<Condition>, crate::ParseArgsError> {
...
fn parse_regex(bytes: &[u8]) -> Result<regex::bytes::Regex, crate::ParseArgsError> {
    let str = match std::str::from_utf8(bytes) {
        Ok(v) => Ok(v),
        Err(e) => Err(crate::ParseArgsError::invalid_field(e)),
    }?;
...
fn constant_literal_to_regex(
    bytes: &[u8],
) -> Result<regex::bytes::Regex, crate::ParseArgsError> {
...
) -> Result<Option<ContentsMatchCondition>, crate::ParseArgsError> {
...
) -> Result<Vec<Condition>, crate::ParseArgsError> {
...
fn get_contents_match_conditions(
    proto: &[rrg_proto::flows::FileFinderCondition],
) -> Result<Vec<ContentsMatchCondition>, crate::ParseArgsError> {
    let mut conditions = vec![];
    for proto_condition in proto {
...
impl crate::Args for Request {
    ...
    fn from_proto(proto: rrg_proto::flows::FileFinderArgs) -> Result<Request, crate::ParseArgsError> {
        ...
        if proto.get_pathtype() != rrg_proto::jobs::PathSpec_PathType::OS {
            ...
            return Err(crate::ParseArgsError::invalid_field(error));
        }
...
mod tests {
    ...
    fn test_fails_on_non_os_path_type() {
        ...
        match err.kind() {
            crate::ParseArgsErrorKind::InvalidField => {}
    ...
    fn test_invalid_utf8_sequence_error() {
        ...
        assert!(matches!(err.kind(), crate::ParseArgsErrorKind::InvalidField));
    }
